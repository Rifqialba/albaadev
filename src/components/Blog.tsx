...
export default function Blog({ data, tags, itemsPerPage = 5 }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [filteredPosts, setFilteredPosts] = createSignal<CollectionEntry<"blog">[]>([])
  const [currentPage, setCurrentPage] = createSignal(1)
  const [paginatedPosts, setPaginatedPosts] = createSignal<CollectionEntry<"blog">[]>([])

  // Hitung total halaman berdasarkan data yang sudah difilter
  const totalPages = () => Math.ceil(filteredPosts().length / itemsPerPage)

  // Update filtered posts ketika filter berubah
  createEffect(() => {
  ...
  // Update paginated posts ketika filteredPosts atau currentPage berubah
  createEffect(() => {
